      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2">
        {ideas.map((idea, index) => (
          <div
            ...
            className="bg-white backdrop-blur-sm bg-white/90 rounded-xl p-6 shadow-lg hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300 border border-gray-100 relative cursor-pointer"
          >
<button
type="button"
...
<li
key={idx}
className={`mb-1 ${todo.completed ? "line-through text-gray-500" : ""}`}
>
{todo.task}
</li>
))}
      ...
      {selectedIdea && (
        ...
        >
          ...
          >
            ...
            <p className="text-black mb-6">{selectedIdea.description}</p>
            <p className="text-gray-600 mb-6">{selectedIdea.details}</p>
            <div className="flex flex-wrap items-center mb-6">
              {selectedIdea.languages.map((language, idx) => (
                <span
                  key={idx}
                  className="inline-flex items-center px-3 py-1.5 mr-2 mb-2 rounded-full text-sm font-medium bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-sm"
                >
              ...
              {selectedIdea.frameworks.map((framework, idx) => (
                <span
                  key={idx}
                  className="inline-flex items-center px-3 py-1.5 mr-2 mb-2 rounded-full text-sm font-medium bg-gradient-to-r from-green-500 to-green-600 text-white shadow-sm"
                >
