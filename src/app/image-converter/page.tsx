...
const imageConverterModule = await import('../../lib/image-converter/image_converter');
      ...
      // 各フォーマットに変換
      const formats = [
        { name: 'PNG', func: () => converter.toPNG(inputString), mime: 'image/png' },
        { name: 'JPEG', func: () => converter.toJPEG(inputString, 90), mime: 'image/jpeg' },
        { name: 'BMP', func: () => converter.toBMP(inputString), mime: 'image/bmp' },
        { name: 'TGA', func: () => converter.toTGA(inputString), mime: 'image/x-tga' },
      ];
        ...
        <div className="mt-12">
          ...
          >
            ...
            <div className="flex flex-col items-center">
              ...
              />
              {isDragActive ? (
                <p className="text-lg text-blue-500">ここにドロップしてください</p>
              ) : (
            ...
            <div className="mt-8 p-6 bg-white rounded-lg shadow">
              ...
              <dl className="mt-4 grid grid-cols-3 gap-4">
                <div>
                  ...
                  <dd className="mt-1 text-lg text-gray-900">{imageInfo.width}px</dd>
                </div>
                <div>
                  <dt className="text-sm text-gray-500">高さ</dt>
                  <dd className="mt-1 text-lg text-gray-900">{imageInfo.height}px</dd>
                </div>
                <div>
                  <dt className="text-sm text-gray-500">チャンネル数</dt>
                  <dd className="mt-1 text-lg text-gray-900">{imageInfo.channels}</dd>
                </div>
              </dl>
            </div>
          )}
