    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 py-2">
            <div className="bg-white p-8 rounded-2xl shadow-lg">
                <h1 className="text-4xl font-bold mb-6 text-center text-gray-800">ログイン</h1>
                {Object.values(providers).map((provider: any) => (
                    <div key={provider.name} className="w-full mb-4">
                        ...
                        >
                            <img src={`https://skillicons.dev/icons?i=${provider.name.toLowerCase()}`} alt={`${provider.name} icon`} className="w-6 h-6 mr-2" />
                            {provider.name}でログイン
                        </button>
                    </div>
                ))}
